...
export async function action(opts: ActionOptions) {
  ...
  try {
    ...
    const aContent = await readFile(
      path.resolve(CWD, tempDirPath, "json/a-tokens.json"),
      { encoding: "utf-8" }
    );
    const bContent = await readFile(
      path.resolve(CWD, tempDirPath, "json/shorthands-tokens.json"),
      {
        encoding: "utf-8",
      }
    );
    ...
    await writeFile(
      path.resolve(CWD, opts.output, "json/sprout-tokens.json"),
