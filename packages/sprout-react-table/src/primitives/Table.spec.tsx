...
test("should manage overflow from the container", async ({ mount, page }) => {
  ...
  expect(Math.floor(cellBB?.width || 0)).toBeGreaterThan(
    Math.floor(secondCellBB?.width || 0)
  );

  // now let's go under the minimum width to check overflow is ok
  await component.update(
    <TableTest width={MIN_TABLE_WIDTH - 50} columnWidth={CELL_MIN_WIDTH} />
  );
  ...
  await component.update(
    <TableTest width={MIN_TABLE_WIDTH} columnWidth={CELL_MIN_WIDTH} />
  );
  tableBB = await table.boundingBox();
...
test("should support min/max width and flex value", async ({ mount, page }) => {
  ...
  const widths = await Promise.all<Array<Promise<number>>>(
    headers.map((header) =>
      header.evaluate((el) => (el as HTMLElement).offsetWidth)
    )
  );
...
test("DLS-442: should remove the padding when no horizontal scroll", async ({
  mount,
  page,
}) => {
  ...
  const hasInitialHorizontalScroll = await scroller.evaluate(
