...
import { type BoxSizing, heightObserver } from "../utils/heightObserver";
    ...
    const observer = heightObserver(container, {
      ...
      onResize: (newHeightParam: number | undefined): void => {
        ...
        if (newHeight > MAX_HEIGHT) {
          ...
          console.error(
            "AutoSizer: Detected height is too large, capping to prevent issues."
          );
        ...
        const minHeightStyle = parseFloat(
          computedStyle.getPropertyValue("min-height")
        );
        const maxHeightStyle = parseFloat(
