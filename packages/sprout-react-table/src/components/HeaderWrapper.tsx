...
import type { TableProps } from "@qlik/sprout-react";
...
export function HeaderWrapper<ContentType>({
  info,
  colDef,
  prefixCount,
  suffixCount,
  children,
  ...props
}: HeaderWrapperProps<ContentType>) {
  const ref = useRef<HTMLDivElement>(null);
  const visibleCols = info.table.getVisibleFlatColumns();
  const visibleUserCols = visibleCols.filter(
    (tColumn, i) =>
      i >= (prefixCount || 0) && i < visibleCols.length - (suffixCount || 0)
  );
  ...
  const isHidingEnabled: boolean =
    selectionColumn.getCanHide() && visibleUserCols.length > 1;

  const hasActions: boolean =
    isSortingEnabled ||
    isHidingEnabled ||
    info.table.options.enableHiding ||
    colDef.headerContextMenu !== undefined;
          ...
          <TableHTML.HeaderMenuButton
            ...
            menu={
              <TableHTML.HeaderMenuContent
                sortOrder={
                  isSortingEnabled ? selectionColumn.getIsSorted() : undefined
                }
                onSortChange={
                  isSortingEnabled
                    ? (sort: "asc" | "desc") => {
