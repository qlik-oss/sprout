...
import {
  TableFillParentContainer,
  TableFlexContainerAsyncTest,
  TableFlexParentContainerHeight,
} from "./AutoSizerTest";

test("should take the full height of a flex container when data is loading asynchronously", async ({
  mount,
  page,
}) => {
  ...
  const tableContainerHeight = await tableContainer.evaluate(
    (el) => el.clientHeight
  );
  expect(tableContainerHeight).toBe(
    containerHeight - headerHeight - footerHeight
  );
});

test("should fill the height of the parent container", async ({
  mount,
  page,
}) => {
...
test("should table take needed space in flex column", async ({
  mount,
  page,
}) => {
  ...
  await component.update(
    <TableFlexParentContainerHeight style={{ maxBlockSize: 200 }} />
  );
  await page.waitForTimeout(10);
  const tableHeightWithMaxHeight = await table.evaluate(
    (el) => el.clientHeight
  );
  ...
  await component.update(
    <TableFlexParentContainerHeight style={{ minBlockSize: 400 }} />
  );
  await page.waitForTimeout(10);
  const tableHeightWithMinHeight = await table.evaluate(
    (el) => el.clientHeight
  );
