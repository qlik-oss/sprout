...
import {
  type ColumnSelectionTable,
  type ColumnSelectionTableOptions,
...
export function Table<ContentType>({
  ...
  const [loadingRows, setLoadingRows] = useState<Array<ContentType>>(
    new Array(50)
      .fill(null)
      .map((_, i) => ({ id: i }) as unknown as ContentType)
  );
  ...
  const { columnVisibilityControl, columnSizingControl, sortState, headerRef } =
    useTableState<ContentType>({
      tColumns,
      columnVisibility,
      onColumnVisibilityChange,
      defaultColumnVisibility,
  ...
  const columnSelectionControl = useControlled<
    Pick<
      TableProps<ContentType>,
      "columnSelection" | "onColumnSelectionChange" | "defaultColumnSelection"
    >
  >(
    {
      columnSelection,
  ...
  const toggledRowControl = useControlled<
    Pick<
      TableProps<ContentType>,
      "defaultToggledRowId" | "toggledRowId" | "onToggledRowChange"
    >
  >(
  ...
  const getEnableRowSelectionConfig = useCallback(():
    | boolean
    | ((row: Row<ContentType>) => boolean) => {
    if (!rowSelectionMode) {
      return false;
    }
  ...
  const tableOptions = {
    ...
    data: safeRows,
    getRowId: loading
      ? (row: ContentType) => (row as ContentType & { id: string }).id || ""
      : getRowId,
    columns: tColumns,
    ...
    getSortedRowModel:
      sortingMode === "server" ? undefined : getSortedRowModel(),
    state: {
    ...
    onColumnVisibilityChange:
      columnVisibilityControl.onColumnVisibilityChange as OnChangeFn<
        Record<string, boolean>
      >,
    onColumnSizingChange:
      columnSizingControl.onColumnSizingChange as OnChangeFn<ColumnSizingState>,
    onColumnSelectionChange:
      columnSelectionControl.onColumnSelectionChange as OnChangeFn<string>,
    onColumnsSelectionChange:
      columnsSelectionControl.onColumnsSelectionChange as OnChangeFn<
        Array<string>
      >,
  ...
  const table = useReactTable<ContentType>(
    tableOptions
  ) as ColumnSelectionTable<ContentType>;

  const prefixCount =
    (rowSelectionMode ? 1 : 0) + (displayRowNumberColumn ? 1 : 0);
  const suffixCount = rowActions ? 1 : 0;
  const isToggledStateEnabled =
    !!defaultToggledRowId || !!toggledRowId || !!onToggledRowChange;

  const columnVisibilityView = useColumnVisibilityView({
    headerRef,
    onColumnVisibilityChange: (subColumns) => {
      const newState = subColumns.reduce<Record<string, boolean>>(
        (acc, column) => {
          if (column.id) {
    ...
    columns: table
      ...
      .filter(
        (tColumn, i) =>
          i > prefixCount - 1 && i < table.getAllColumns().length - suffixCount
      )
      .map((tColumn, i) => {
  ...
  const rowVirtualizer = useVirtualizer<HTMLDivElement, HTMLDivElement>({
    ...
    getScrollElement: () =>
      ...
      ) as HTMLDivElement, //get scroll element from parent of table container
    measureElement:
            ...
            {safeRows.length === 0 ? (
              ...
              <TableHTML.Body>
                ...
                {items.map((virtualRow) => {
                  ...
                  const row = rowModel.rows[
                    virtualRow.index
                  ] as Row<ContentType>;
                  const rowId = row.id || getRowId?.(row.original);
                  const isToggled = isToggledStateEnabled
                    ? toggledRowControl.toggledRowId === rowId
                    : false;
