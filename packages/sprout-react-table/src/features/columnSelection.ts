...
export type ColumnSelectionTableOptions<TData extends RowData> = Omit<
  TableOptions<TData>,
  "state"
> &
...
export type ColumnSelectionColumn<
  ...
  TValue = unknown,
> = Column<TData, TValue> & ColumnSelectionColumnAPI;

export type ColumnSelectionHeaderContext<
  TData extends RowData,
  TValue = unknown,
...
export type ColumnSelectionCellContext<
  TData extends RowData,
  TValue = unknown,
...
export const columnSelectionFeature: TableFeature<any> = {
  ...
  getDefaultOptions: <TData extends RowData>(
    table: Table<TData>
  ): ColumnSelectionTableOptions<TData> => {
  ...
  createColumn: <TData extends RowData>(
    ...
    selectionColumn.getSelectionMode = () => {
      const columnMeta = selectionColumn.columnDef.meta as
        | ColumnSelectionColumnMeta
        | undefined;
      return columnMeta?.selectable === false
        ? undefined
        : selectionTable.options.columnSelection;
    };
    selectionColumn.isSelected = () => {
      const mode = selectionColumn.getSelectionMode();
      if (!mode) return false;
      const selectionState =
        selectionTable.getState() as ColumnSelectionTableState;
      if (mode === "multiple") {
    ...
    selectionColumn.toggleSelection = () => {
      ...
      if (mode === "single") {
        ...
        selectionTable.options.onColumnSelectionChange?.(
          isSelected ? "" : colId
        );
      } else {
        const current = selectionState.columnsSelected;
        if (current.includes(colId)) {
          selectionTable.options.onColumnsSelectionChange?.(
            current.filter((id: string) => id !== colId)
          );
        } else {
          selectionTable.options.onColumnsSelectionChange?.([
            ...current,
