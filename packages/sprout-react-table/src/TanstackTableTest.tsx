...
export function TableCustomSortingTest(props: Partial<TableProps<Entity>>) {
  ...
  return (
    <Table
      variant="data"
      columns={entityCustomSorting}
      rows={rows as Array<Entity>}
      {...props}
    />
  );
}
...
export function ExternalRowSelectionTest() {
  ...
  return (
    ...
    >
      <div
        className={classNames("flex", "flex-row", "w-full", "justify-between")}
      >
        <div
          className={classNames("flex", "flex-row", "justify-start", "mb-m")}
        >
          <TextField
            name="search"
            placeholder="Search..."
        ...
        <div className={classNames("flex", "flex-row", "justify-end", "mb-m")}>
          ...
          <Button
            ...
            onClick={() => {
              setRows((prevRows) =>
                prevRows.filter((row) => !selected[row.uuid])
              );
              handleRowSelection({});
            }}
          />
...
export function DisabledRowSelectionTest() {
  const isRowSelectable = useCallback(
    (row: Person) => row.firstName !== "Trace",
    []
  );
...
export function OnCellClickTest() {
  ...
  return (
    <div>
      <ul
        style={{ blockSize: 200, overflowBlock: "auto" }}
        data-testid="clicked-cell-list"
      >
      ...
      <Table
        ...
        onCellClick={(e, param) => {
          setClickedCell((prev) => ({
            ...prev,
            [`${param.row.uuid}-${param.value}`]:
              (prev[`${param.row.uuid}-${param.value}`] || 0) + 1,
          }));
        }}
      />
    </div>
...
export function RowActionsTest() {
  ...
  return (
    <div className={classNames("w-full", "p-xl", "border-box")}>
      <span data-testid="counter">{count}</span>
      <span data-testid="item-clicked">
...
export function TableFlexColumnSize({
  totalWidth,
}: {
  ...
  return (
    <div
      ...
      style={{ inlineSize: totalWidth, blockSize: 400 }}
    >
      <Table
        variant="data"
        rowHeight={48}
...
export function TableWithPanelOverlay() {
  return (
    <div style={{ inlineSize: "100vw", blockSize: "100vh" }}>
      <div
        className={classNames("flex", "flex-row", "w-3xl", "border-r-default")}
        style={{ blockSize: 400 }}
      >
...
export function ControlledColumnSizingTableTest({
  disableColumnResize,
}: {
  ...
  return (
    <div>
      <pre data-testid="column-sizing-state">
