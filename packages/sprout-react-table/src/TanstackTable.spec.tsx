...
import { type Person, makePerson, personColDef } from "./docs/examples/person";
...
test("should support sort feature", async ({ page, mount }) => {
  ...
  await expect(
    firstHeaderMenu.getByRole("menuitem", { name: "Sort ascending" })
  ).toBeVisible();
  await expect(
    firstHeaderMenu.getByRole("menuitem", { name: "Sort descending" })
  ).toBeVisible();
  await expect(
    firstHeaderMenu.getByRole("menuitem", { name: "Sort ascending" })
  ).toBeDisabled();
  ...
  await secondHeader
    .getByRole("button", { name: "Contextual actions" })
    .click();
  ...
  await secondHeaderMenu
    .getByRole("menuitem", { name: "Sort descending" })
    .click();
  await expect(secondHeader).toHaveAttribute("aria-sort", "descending");
});

test("should support possibility to disable sorting", async ({
  page,
  mount,
}) => {
  ...
  await expect(
    nameColumnMenu.getByRole("menuitem", { name: "Sort ascending" })
  ).toBeVisible();
  await expect(
    nameColumnMenu.getByRole("menuitem", { name: "Sort descending" })
  ).toBeVisible();
  // Click outside to close the menu
  await page.keyboard.press(KEYBOARD_KEYS.ESCAPE);
  ...
  await expect(
    typeColumnMenu.getByRole("menuitem", { name: "Sort ascending" })
  ).toBeVisible();
  await expect(
    typeColumnMenu.getByRole("menuitem", { name: "Sort descending" })
  ).toBeVisible();
  // Click outside to close the menu
  await page.keyboard.press(KEYBOARD_KEYS.ESCAPE);
  ...
  await expect(
    modifiedColumnMenu.getByRole("menuitem", { name: "Sort ascending" })
  ).not.toBeVisible();
  await expect(
    modifiedColumnMenu.getByRole("menuitem", { name: "Sort descending" })
  ).not.toBeVisible();
  // Click outside to close the menu
  await page.keyboard.press(KEYBOARD_KEYS.ESCAPE);
  ...
  await expect(
    spaceColumnMenu.getByRole("menuitem", { name: "Sort ascending" })
  ).not.toBeVisible();
  await expect(
    spaceColumnMenu.getByRole("menuitem", { name: "Sort descending" })
  ).not.toBeVisible();
  // Click outside to close the menu
  await page.keyboard.press(KEYBOARD_KEYS.ESCAPE);
...
test("should support possibility to use custom sorting", async ({
  page,
  mount,
}) => {
  ...
  await expect(rows[1].getByRole("gridcell").nth(1)).toContainText(
    "2024-01-01"
  );
  await expect(rows[2].getByRole("gridcell").nth(1)).toContainText(
    "2025-01-01"
  );
  await expect(rows[3].getByRole("gridcell").nth(1)).toContainText(
    "2026-01-01"
  );
...
test("should support no rows", async ({ mount }) => {
  const mounted = await mount(
    <Table variant="data" columns={personColDef} rows={[]} />
  );
  const rows = await mounted.getByRole("row").all();
...
test("should keep noRowsOverlay centered in the visible area with many columns", async ({
  page,
  mount,
}) => {
  const wideColumns = personColDef.map((column) => ({
    ...column,
    width: 350,
  }));

  await mount(
    <div style={{ inlineSize: 420, blockSize: 320 }}>
      <Table variant="data" columns={wideColumns} rows={[]} />
    </div>
  );

  const scroller = page.getByRole("grid").locator("..");
  const overlay = page.getByRole("heading", { name: "There are no rows" });

  await expect(overlay).toBeVisible();

  const hasHorizontalScroll = await scroller.evaluate(
    (el) => el.scrollWidth > el.clientWidth
  );
  expect(hasHorizontalScroll).toBe(true);

  const scrollerBB = await scroller.boundingBox();
  const overlayBB = await overlay.boundingBox();

  if (!scrollerBB || !overlayBB) {
    throw new Error(
      "Unable to compute bounding boxes for noRowsOverlay regression check"
    );
  }

  const scrollerCenter = scrollerBB.x + scrollerBB.width / 2;
  const overlayCenter = overlayBB.x + overlayBB.width / 2;

  expect(Math.abs(scrollerCenter - overlayCenter)).toBeLessThanOrEqual(24);
});
...
test.describe("Row selection", (): void => {
  ...
  test("should support disabling selection for specific rows", async ({
    mount,
    page,
  }) => {
    ...
    const rowWithEnabledSelection = rowsContainer
      .getByRole("row")
      .filter({ hasText: "Yadira" });
    const rowWithDisabledSelection = rowsContainer
      .getByRole("row")
...
test.describe("Row toggling", (): void => {
  ...
  for (const variant of variants) {
    ...
    test(`should work with ${variant} variant without row selection`, async ({
      mount,
      page,
    }): Promise<void> => {
      await mount(
      ...
      await expect(thirdRow).toHaveCSS(
        "background-color",
      ...
      await expect(thirdRow).not.toHaveCSS(
        "background-color",
        toggledRowBackground
      );
      // AND the newly clicked row is toggled
      await expect(seventhRow).toHaveCSS(
        "background-color",
        toggledRowBackground
      );
      ...
      await expect(seventhRow).not.toHaveCSS(
        "background-color",
        toggledRowBackground
      );
  ...
  for (const variant of variants) {
    // eslint-disable-next-line @typescript-eslint/no-loop-func
    test(`should work with ${variant} variant with row selection`, async ({
      mount,
      page,
    }): Promise<void> => {
      await mount(
  ...
  test("should support controlled row toggling", async ({ mount, page }) => {
    ...
    await page.mouse.move(0, 0);
    // THEN the row is untoggled
    await expect(fifthRow).not.toHaveCSS(
      "background-color",
      toggledRowBackground
    );
...
test("should support external selection management", async ({
  mount,
  page,
}) => {
...
test("should support resizing columns", async ({ mount, page }) => {
  ...
  const headers = await page.getByRole("columnheader").all();
  const firstHeader = headers[0];
  const firstHeaderResize = await firstHeader.getByRole("button", {
    name: "Resize column",
  });
...
test("should support controlled column resizing", async ({ mount, page }) => {
  ...
  const modifiedHeader = headers[2];
  const spaceHeader = headers[3];
  const nameHeaderBB = await nameHeader.boundingBox();
  ...
  const nameHeaderResize = await nameHeader.getByRole("button", {
    name: "Resize column",
  });
  await nameHeader.hover();
  ...
  // THEN visual column width should update during drag (columnResizeMode is "onChange")
  // Wait a frame for the DOM to update after mouse move
  await page.waitForTimeout(10);
  const visualNameHeaderBB = await nameHeader.boundingBox();
  expect(visualNameHeaderBB?.width).toBeGreaterThan(430);
  expect(visualNameHeaderBB?.width).toBeLessThan(450);

  // BUT controlled state callback should not fire yet (debounced with 50ms timeout)
  // Check immediately before debounce fires
  const pendingColumnSizingStateElement = await page.getByTestId(
    "column-sizing-state"
  );
  const pendingControlledColumnSizingState =
    await pendingColumnSizingStateElement.textContent();
  const pendingMatch =
    pendingControlledColumnSizingState &&
    pendingControlledColumnSizingState.match(/"name":\s*(\d+)/);
  const pendingNameColumnWidth = pendingMatch ? Number(pendingMatch[1]) : 0;
  expect(pendingNameColumnWidth).toBe(300);

  await page.mouse.up();

  // AFTER mouse up, wait for the debounced callback to fire and controlled state to update
  // The callback uses a 50ms debounce to detect drag end
  await page.waitForTimeout(100);

  // AFTER mouse up, the callback fires and controlled state updates
  const columnSizingStateElement = await page.getByTestId(
    "column-sizing-state"
  );
  const controlledColumnSizingdState =
    await columnSizingStateElement.textContent();
  const match =
    controlledColumnSizingdState &&
    controlledColumnSizingdState.match(/"name":\s*(\d+)/);
...
test("should support possibility to disable column resizing", async ({
  mount,
  page,
}) => {
  ...
  const typeHeaderResize = await typeHeader.getByRole("button", {
    name: "Resize column",
  });
  const modifiedHeaderResize = await modifiedHeader.getByRole("button", {
    name: "Resize column",
  });
  const spaceHeaderResize = await spaceHeader.getByRole("button", {
    name: "Resize column",
  });
...
test("should support rowActions", async ({ mount, page }) => {
  ...
  // right click on menu item should not open a second menu
  await actionButton.click();
  await expect(menu).toBeVisible();
  await actionItem.click({ button: "right" });
  await expect(menu).toBeVisible();
...
test("should support right click context menu rowActions", async ({
  mount,
  page,
}) => {
  await mount(<RowActionsTest />);
...
test("should close context menu rowActions on scroll", async ({
  mount,
  page,
}) => {
  await mount(<RowActionsTest />);
...
test("should support headerContextMenu", async ({ mount, page }) => {
  ...
  const firstHeader = headers[0];
  await firstHeader.hover();
  const contextBtn = await firstHeader.getByRole("button", {
...
test("should support possibility to disable hiding", async ({
  page,
  mount,
}) => {
  ...
  await expect(nameColumnMenu).toBeVisible();
  await expect(
    nameColumnMenu.getByRole("menuitem", { name: "Hide column" })
  ).not.toBeVisible();
  await expect(
    nameColumnMenu.getByRole("menuitem", { name: "Manage columns" })
  ).not.toBeVisible();
});

test("should not show menu if no actions are available", async ({
  page,
  mount,
}) => {
...
test("should not trigger sorting when resizing a column", async ({
  page,
  mount,
}) => {
  await mount(<TableTest />);
...
test("should support the loading property", async ({ mount, page }) => {
  ...
  const allSkeletons = await page
    .locator('[data-variant="text"][aria-hidden="true"]')
    .all();
  expect(allSkeletons.length).toBe(11 * 21); // col * rows
});

test("should header be displayed under the panel overlay", async ({
  mount,
  page,
}) => {
...
test("should manage scrollbars sizes DLS-502", async ({ mount, page }) => {
  ...
  expect(Math.round(tableBB.height)).toBe(
    Math.round(bodyBB.height + headerBB.height + 2 + 12)
  ); // 2 for borders 12 for scrollbar
  expect(Math.floor(bodyBB.height)).toBe(318); // body should take the rest of the height
});

test.describe("Row in interactive mode", () => {
  test("should not be interactive in list variant without onRowClick or rowSelectionMode", async ({
    mount,
    page,
  }) => {
    ...
    await expect(firstRow).not.toHaveCSS(
      "background-color",
      "rgba(0, 0, 0, 0.03)"
    );
  });

  test("should be interactive in list variant with onRowClick", async ({
    mount,
    page,
  }) => {
  ...
  test("should be interactive in data variant when the row is clickable", async ({
    mount,
    page,
  }) => {
  ...
  test("should not be interactive in data variant when there is row selection", async ({
    mount,
    page,
  }) => {
