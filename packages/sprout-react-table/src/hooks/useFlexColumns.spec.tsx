...
test("should handle column size using fixed with and flex properties", async ({
  mount,
  page,
}) => {
  ...
  const tableContainerOffsetWidth = await tableContainer.evaluate(
    (el) => (el as HTMLElement).offsetWidth
  );
  expect(tableContainerOffsetWidth).toBe(totalWidth);

  // inner width exclude borders
  const tableContainerInnerWidth = await tableContainer.evaluate(
