  ...
  const calculateAndSetWidths = useCallback(() => {
    ...
    if (totalFlexRatio > 0 && remainingWidth > 0) {
      flexColumns.forEach((column) => {
        ...
        newWidths[column.headerName] =
          Math.floor(remainingWidth / totalFlexRatio) * column.flex;
      });
    }
    ...
    // do not put opts as dependency to avoid infinite loop when opts is not memoized
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [containerRef, initialColumnDefs]);
