...
import { type TableProps, classNames } from "@qlik/sprout-react";
...
export const useColumnVisibilityState = ({
  ...
  const columnVisibilityControl = useControlled<
    Partial<ColumnVisibilityControl>
  >(
    {
      columnVisibility,
      onColumnVisibilityChange,
      defaultColumnVisibility:
        defaultColumnVisibility || columnVisibilityDefault,
    },
    {
...
export function useColumnVisibilityView({
  ...
  const maxOffset =
    window.innerWidth > MENU_MAX_WIDTH ? window.innerWidth - MENU_MAX_WIDTH : 0;
